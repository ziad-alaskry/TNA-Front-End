...
const mockTasks: DriverTask[] = [
    { id: 'TSK-01', tna_code: 'TNA-667722', customerKey: 'task_1_customer', addressKey: 'task_1_address', timeKey: 'task_1_time', status: 'NAVIGATING', priority: 'HIGH' },
    { id: 'TSK-02', tna_code: 'TNA-102938', customerKey: 'task_2_customer', addressKey: 'task_2_address', timeKey: 'task_2_time', status: 'PENDING', priority: 'NORMAL' },
    { id: 'TSK-03', tna_code: 'TNA-229911', customerKey: 'task_3_customer', addressKey: 'task_3_address', timeKey: 'task_3_time', status: 'PENDING', priority: 'NORMAL' },
]
        ...
        <AppShell role="Carrier" header={t('carrier.driver.tasks.header')}>
            <div className="space-y-6">
                <div className="flex items-center gap-4 p-6 bg-surface-200 rounded-md border border-neutral-200">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-neutral-900">{t('carrier.driver.tasks.driver_name')}</h2>
                        <p className="text-xs text-neutral-500 font-medium mt-1">{t('carrier.driver.tasks.today')}</p>
                    </div>
                </div>
                    ...
                    {mockTasks.map((task) => (
                        <div key={task.id} className={`p-4 rounded-md border transition-all ${
                            ...
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <span className="text-[10px] font-bold text-neutral-400">{t(`carrier.driver.tasks.${task.timeKey}`)}</span>
                            </div>
                            ...
                            <div className="space-y-3">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex flex-col">
                                        <span className="text-sm font-bold text-neutral-900">{t(`carrier.driver.tasks.${task.customerKey}`)}</span>
                                        <span className="text-xs text-neutral-500 leading-relaxed">{t(`carrier.driver.tasks.${task.addressKey}`)}</span>
                                    </div>
                                </div>
                            </div>
                                ...
                                {task.status === 'NAVIGATING' ? (
                                    <>
                                        ...
                                        >
                                            ...
                                            {t('carrier.driver.tasks.continue_navigation')}
                                        </button>
                                    ...
                                    <>
                                        <button className="flex-1 h-11 bg-primary text-white text-sm font-bold rounded-sm hover:bg-primary-dark transition-colors">
