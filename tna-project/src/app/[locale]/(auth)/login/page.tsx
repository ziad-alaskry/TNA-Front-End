...
import { MapPin, House, Truck, Buildings, User as UserIcon, Lock, Eye, EyeSlash, SignIn, ShieldCheck } from '@phosphor-icons/react';
...
export default function LoginPage() {
    ...
    return (
        <div className="font-display min-h-screen flex items-center justify-center p-4 bg-surface-100" style={{
            ...
            <div className="w-full max-w-[480px] bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden relative z-10 transition-all duration-300 transform scale-100">
                {/* Top Section: Header */}
                <div className="pt-10 pb-6 px-6 text-center backdrop-blur-md bg-white/80 relative">
                ...
                <div className="px-6 pb-10 flex flex-col gap-4 animate-in fade-in slide-in- duration-500">
                     ...
                     <div className="flex flex-col gap-2">
                         <label className="text-sm font-medium text-slate-700 pr-1">{t('auth.login.role_label')}</label>
                        <div className="bg-slate-50/50 p-1 flex gap-1 rounded-lg border border-slate-200 shadow-sm">
                            {roles.map((r) => (
                                ...
                                >
                                    <span className={`transition-all duration-200 ${role === r.id ? 'text-primary text-[20px] font-bold' : 'text-slate-400 text-[18px]'}`}>
                                        <r.icon size={20} weight={role === r.id ? 'fill' : 'regular'} />
                                    </span>
                                    <span>{r.label}</span>
                                </button>
                            ))}
                     ...
                     {/* Input 1: Username */}
                     <div className="flex flex-col gap-2">
                         <label className="text-sm font-medium text-slate-700 pr-1">{t('auth.login.username_label')}</label>
                        <div className="relative group">
                            ...
                            <input 
                                 className="w-full h-[56px] pr-12 pl-4 rounded-lg border border-slate-200 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all bg-slate-50/50 text-slate-900 placeholder:text-slate-400 outline-none" 
                     ...
                     {/* Input 2: Password */}
                     <div className="flex flex-col gap-2">
                         <label className="text-sm font-medium text-slate-700 pr-1">{t('auth.login.password_label')}</label>
                        <div className="relative group">
                            <div className="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none text-slate-400 group-focus-within:text-primary transition-colors">
                                <Lock size={20} />
                            </div>
                    <input 
                        className="w-full h-[56px] pr-12 pl-12 rounded-lg border border-slate-200 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all bg-slate-50/50 text-slate-900 placeholder:text-slate-400 outline-none" 
                    ...
                    >
                        {showPassword ? <EyeSlash size={20} /> : <Eye size={20} />}
                         ...
                         <div className="flex justify-start">
                             <Link href={`/${locale}/forgot-password`} className="text-xs font-semibold text-primary hover:underline transition-colors">
                    ...
                    <div className="mt-4 flex flex-col gap-4">
                        ...
                        <button 
                            onClick={handleLogin}
                            className="w-full h-[56px] rounded-full text-white font-bold text-lg shadow-lg shadow-primary/20 flex items-center justify-center gap-2 hover:opacity-90 transition-opacity"
                            style={{ background: 'linear-gradient(to left, #0CBBDB, #1A73C1)' }}
                        >
                            <SignIn size={24} weight="bold" />
                            <span>{t('auth.login.login_button', { role: currentRoleLabel })}</span>
                        </button>

                        {/* Divider */}
                        <div className="relative flex items-center py-4">
                            <div className="flex-grow border-t border-slate-200"></div>
                            <span className="flex-shrink mx-4 text-slate-400 text-sm font-medium">{t('auth.login.or_divider')}</span>
                            <div className="flex-grow border-t border-slate-200"></div>
                        </div>

                        {/* Nafath SSO */}
                        <button className="w-full h-[56px] rounded-full border-2 border-primary bg-white text-primary font-bold text-base flex items-center justify-center gap-3 hover:bg-primary/5 transition-colors">
                            <div className="size-6 bg-[#00a651] rounded-md flex items-center justify-center text-white text-[10px]">
