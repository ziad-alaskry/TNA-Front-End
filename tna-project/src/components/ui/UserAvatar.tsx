...
export function UserAvatar({ isVisitor = true, user, src, alt, className, onClick }: UserAvatarProps) {
  ...
  if (onClick) {
    return (
      <button 
        onClick={onClick}
        className={cn('relative flex items-center justify-center rounded-full hover:opacity-80 transition-opacity', className)}
      >
        {content}
      </button>
    );
