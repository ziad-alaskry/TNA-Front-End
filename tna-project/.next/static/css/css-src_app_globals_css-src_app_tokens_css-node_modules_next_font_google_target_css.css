...
[hidden]:where(:not([hidden="until-found"])) {
  ...
  :root {
    ...
    /* Sidebar legacy mapping (can be refined) */
...
/* UI DNA: All cards/containers must match SPATIAL spec */
.sr-only {
  position: absolute;
  width: 1px;
  height: 1px;
...
.border-t-brand-navy {
  border-top-color: var(--primitive-navy);
}
.bg-amber-100 {
  --tw-bg-opacity: 1;
...
.text-error-bg {
  --tw-text-opacity: 1;
  color: rgb(253 236 234 / var(--tw-text-opacity, 1));
}
.text-gray-400 {
  --tw-text-opacity: 1;
